...
const PhoneField = (props) => {
    ...
    const value = (lodashExports.get(formikProps, `values.${name}`) || "");
    useEffect(() => {
        if (value) {
...
const getConditionalOutput = (itemCondition, formikProps) => {
    const itemValue = lodashExports.get(formikProps, `values.${itemCondition.key}`);
...
const hasTruthyValue = (logicalOperation = "AND", values, formikProps) => {
    ...
    lodashExports.forEach(values, (item, index) => {
...
const getConditionalProps = (itemConfig, formikProps) => {
    ...
    if (!conditionInstructions || lodashExports.isEmpty(conditionInstructions.values)) {
        return { finalProps: {} };
    }
...
const setDefaultProps = (type, props) => {
    if (lodashExports.isArray(type)) {
        lodashExports.map(type, (item) => (ComponentMapConfig[item].props = {
...
const BuildFormRow = (props) => {
    ...
    const columnItems = lodashExports.get(schema, "columns");
    const rowSettings = {
        ...settings,
    };
    const colItems = lodashExports.isArray(schema)
        ? schema
        : lodashExports.isArray(columnItems)
            ? columnItems
            : [schema];
    const rowStyle = { marginBottom: rowSettings.verticalSpacing || 10 };
    return (React.createElement("div", { className: "row", style: rowStyle }, lodashExports.map(colItems, (item, index) => {
...
const getUpdateSchema = (schema, formId) => {
    return lodashExports.map(schema, (schemaItem) => {
        if (lodashExports.isArray(schemaItem)) {
            return lodashExports.map(schemaItem, (item) => ({
                ...item,
                id: `${formId}_${lodashExports.uniqueId()}`,
            }));
        }
        return { ...schemaItem, id: `${formId}_${lodashExports.uniqueId()}` };
    });
};
const MLFormContent = (props) => {
    ...
    return (React.createElement(React.Fragment, null, lodashExports.map(formSchema, (configRow, index) => {
...
export { ArrayField, BuildFormRow, COUNTRY_LIST, CheckBox, ComponentMapConfig$1 as ComponentMapConfig, FileInput, HelperText, MLFormAction, MLFormBuilder, MLFormContent, PhoneField, PlainText, Radio, ReactForm, SelectField, Switch, TextField, attachField, index as default, getComponentConfig, getFieldError, getMenuOptions, processFilesWithCallback, setDefaultProps, setValue };
